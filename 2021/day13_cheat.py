...
def main():
    with open(os.path.join(os.path.dirname(__file__), "day13_data.txt")) as f:
    ...
    for instruction in instruction_data.split("\n"):
        ...
        value = int(value)
        # print_count(paper, instruction)
        paper.fold(axis, value)

    # print_count(paper, "FINAL")
    print(paper)

steps = 0
def print_count(paper, axis):
    global steps
    print('_' * 20)
    print(axis)
    print(steps, paper.count())
    steps += 1
